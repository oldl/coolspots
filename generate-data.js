#!/usr/bin/env node
/**
 * Coolspots - generate-data.js
 * Genere coolspots-data.json pour les communes bruxelloises.
...
const COOLING_POINTS_CSV = path.join(__dirname, 'coolspots_cooling_points.csv');
...
function groupByStreet(osmTrees, osmRoads) {
  const streets = {};
...
function buildCommuneDataset(commune, osmTrees, osmRoads, osmGreenSpaces, osmWaterFeatures, coolingWaterPoints) {
  ...
  const streets = groupByStreet(communeOsmTrees, communeRoads);
...
async function main() {
  console.log(`Generation de coolspots-data.json pour ${REGION_NAME}...`);
  const coolingWaterPoints = loadCoolingWaterPoints();
  const [treesResult, roadsResult, greenSpacesResult, waterResult] = await Promise.all([
  ...
  const communes = COMMUNES.map(commune => buildCommuneDataset(commune, osmTrees, osmRoads, osmGreenSpaces, osmWaterFeatures, coolingWaterPoints));
  const output = {
  ...
  const outPath = path.join(__dirname, 'coolspots-data.json');
